...
import { generateRoleDiscoveryQuestion } from '@/lib/role-discovery';

export const maxDuration = 60;

export async function POST(request: NextRequest) {
    const startTime = Date.now();
    console.log('[API] /api/role-discovery/next-question - Request received');
    
    try {
        const body = await request.json();
        const { cvAnalysis, interviewEvaluation, conversationHistory, language = 'en', questionNumber, totalQuestions } = body;
        ...
        if (!cvAnalysis || !interviewEvaluation) {
            ...
            return NextResponse.json(
                { success: false, error: 'CV analysis and interview evaluation are required' },
                { status: 400 }
            );
        }

        console.log('[API] Calling generateRoleDiscoveryQuestion...');
        const result = await generateRoleDiscoveryQuestion(
            cvAnalysis,
            interviewEvaluation,
            conversationHistory,
            language,
