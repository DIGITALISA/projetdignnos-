...
export default function JobAlignmentPage() {
    ...
    const startAudit = async () => {
        ...
        try {
            ...
            const currentLang = localStorage.getItem('career-upgrade-lang') || 'fr';
    ...
    const submitAudit = async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                // Mark module as used for trial students after completion
                const savedProfile = localStorage.getItem("userProfile");
                const profile = savedProfile ? JSON.parse(savedProfile) : {};
                if (profile.activationType === "Limited") {
                    ...
                    if (userId) {
                        fetch('/api/user/trial-gate', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ userId, module: 'job-alignment', moduleHref: '/job-alignment' })
                        }).then(() => {
    ...
    return (
        <TrialGate 
            ...
            moduleHref="/job-alignment"
            dir={dir}
            manualMark={true}
            language={language}
        >
            <div className={`p-4 md:p-8 max-w-6xl mx-auto space-y-8 pb-32 ${dir === 'rtl' ? 'font-arabic' : ''}`} dir={dir}>
                {/* Header */}
                <header className="space-y-2">
                    ...
                    <h1 className="text-3xl md:text-5xl font-black text-slate-900 tracking-tight">
                        Strategic <span className="text-transparent bg-clip-text bg-linear-to-r from-blue-600 to-indigo-600">{t.jobAlignment.header.titleHighlight}</span>
                    </h1>
                                ...
                                <div className="grid md:grid-cols-2 gap-6">
                                    ...
                                    <div className="hidden md:flex flex-col justify-center p-6 bg-slate-50 rounded-2xl border border-slate-100">
                                        <div className="flex items-center gap-2 text-indigo-600 font-bold mb-2">
                                             <Sparkles size={18} />
                                             <span className="text-xs uppercase tracking-widest">{t.jobAlignment.aiIntelligence.title}</span>
                                         </div>
                                        <p className="text-xs text-slate-500 leading-relaxed italic">
                                             {t.jobAlignment.aiIntelligence.desc}
                                         </p>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="relative group">
                                        ...
                                        <div className="absolute bottom-4 right-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest pointer-events-none">
                                             {t.jobAlignment.form.copyPasteHint}
                                         </div>
                                    </div>
                                </div>
                    ...
                    {step === "questions" && alignment && (
                        ...
                        >
                            ...
                            <div className="flex justify-between items-center bg-slate-900 p-4 px-6 rounded-2xl">
                                <p className="text-slate-400 text-sm font-medium">{t.jobAlignment.questions.hint}</p>
                    ...
                    {step === "result" && alignment && (
                        ...
                        >
                            <div className="flex items-center justify-between flex-wrap gap-4">
                                ...
                                >
                                    <ArrowLeft size={18} />
                                    {t.jobAlignment.result.newAudit}
                                </button>
                                        ...
                                        <div className="relative z-10 space-y-16">
                                            <div className="flex justify-between items-start">
                                                <div className="space-y-1">
                                                    ...
                                                    <h4 className="font-black text-slate-900 transition-tight uppercase tracking-[0.2em] text-xs">{t.jobAlignment.result.academyName}</h4>
                                                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{t.jobAlignment.result.nodeName}</p>
                                                </div>
                                            ...
                                            <div className="space-y-8 text-center">
                                                <div className="space-y-2">
                                                    <span className="inline-block px-4 py-1 bg-blue-100 text-blue-700 text-[10px] font-black rounded-full uppercase tracking-widest border border-blue-200">
                                                        {t.jobAlignment.result.certificateTitle}
                                                    </span>
                                                    <h2 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tight">{t.jobAlignment.result.executiveReport}</h2>
                                                </div>
                                                
                                                <div className="max-w-xl mx-auto space-y-4">
                                                    <p className="text-slate-500 font-bold uppercase tracking-widest text-[11px]">{t.jobAlignment.result.endorsement}</p>
                                            ...
                                            <div className="grid md:grid-cols-12 gap-10 items-center">
                                                <div className="md:col-span-8 space-y-6">
                                                    ...
                                                    <div className="space-y-2">
                                                        <h3 className="font-black text-slate-900 uppercase tracking-widest text-xs">{t.jobAlignment.result.detailedAnalysis}</h3>
                                                ...
                                                <div className="md:col-span-4 flex flex-col items-center justify-center p-8 bg-linear-to-br from-slate-900 to-slate-800 rounded-3xl text-white shadow-xl">
                                                    ...
                                                    <p className="text-[10px] text-center text-slate-400 font-bold uppercase tracking-[0.2em]">{t.jobAlignment.result.verifiedBy}</p>
                                                </div>
                                            </div>
                                            ...
                                            <div className="pt-12 border-t border-slate-100 flex justify-between items-end">
                                                <div className="space-y-1">
                                                    ...
                                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t.jobAlignment.result.issuanceNode}</p>
                                                ...
                                                <div className="space-y-2 text-right">
                                                    ...
                                                    <p className="text-[10px] font-black text-slate-900 uppercase tracking-widest">{t.jobAlignment.result.rectorSignature}</p>
                                                    <p className="text-[9px] text-blue-600 font-black uppercase tracking-widest">{t.jobAlignment.result.encrypted}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
