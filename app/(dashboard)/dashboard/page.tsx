...
export default function DashboardPage() {
  ...
  const [interviewEval, setInterviewEval] = useState<InterviewEval | null>(
    null,
  );
          ...
          try {
            ...
            if (response.hasData && response.data) {
              ...
              if (data.selectedRole) {
                setSelectedRole(
                  typeof data.selectedRole === "string"
                    ? data.selectedRole
                    : data.selectedRole?.title || "",
                );
                localStorage.setItem(
                  "selectedRole",
  ...
  const handleRestart = async () => {
    if (
      confirm(
        t.dashboard.reboot.confirmTitle +
          "\n\n" +
  ...
  const overallScore =
    cvAnalysis?.overallScore || cvAnalysis?.readinessLevel || 0;
  const simScore =
    simulationReport?.overallPerformance?.score ||
    simulationReport?.overallScore ||
    0;
  const seniorityLevel =
    interviewEval?.seniorityLevel || t.dashboard.stats.pendingAnalysis;
  ...
  const steps = [
    ...
    },
    {
      ...
      status:
        isDiagnosisComplete && userPlan === "Student"
          ? "in-progress"
          : "locked",
      icon: BookOpen,
      href: "/library",
    ...
    },
    {
      ...
      status:
        isDiagnosisComplete && userPlan === "Student" ? "completed" : "locked",
      icon: Shield,
  ...
  return (
    <div
      dir={dir}
      className="p-4 md:p-8 max-w-7xl mx-auto space-y-6 md:space-y-10 pb-24 md:pb-8"
    >
      {/* ── Welcome Header ── */}
      <motion.div
      ...
      >
        ...
        {hasStarted && (
          <div
            className={`flex items-center gap-2 bg-blue-600 px-6 py-2.5 rounded-2xl shadow-lg shadow-blue-200 w-fit ${dir === "rtl" ? "flex-row-reverse" : ""}`}
          >
            <TrendingUp className="w-5 h-5 text-white" />
            <span className="font-bold text-sm text-white uppercase tracking-widest">
              {t.dashboard.topLearner}
            </span>
          </div>
        )}
      </motion.div>
        ...
        {[
          {
            ...
            sub: hasStarted
              ? `${technicalSkillsCount} tech · ${softSkillsCount} soft`
              : t.dashboard.stats.startDiagnosis,
            icon: Layers,
            bg: "bg-blue-600/10",
            text: "text-blue-600",
          },
          {
            ...
            sub: hasStarted
              ? cvAnalysis?.rank || t.dashboard.stats.pendingAnalysis
              : t.dashboard.stats.pendingAnalysis,
            icon: BarChart3,
            bg: "bg-purple-600/10",
          ...
          },
          {
            ...
            sub: simScore
              ? t.dashboard.stats.simulationScore
              : t.dashboard.stats.notStartedYet,
            icon: Zap,
            bg: "bg-amber-600/10",
            text: "text-amber-600",
          },
          {
            ...
            value: hasSCI ? "1" : "0",
            sub: hasSCI
              ? t.dashboard.stats.sciReportReady
          ...
          >
            <div
              className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-4 transition-transform group-hover:scale-110 ${stat.bg} ${stat.text}`}
            >
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            {[
              ...
              <motion.div
                key={idx}
                whileHover={
                  !item.locked ? { y: -5, transition: { duration: 0.2 } } : {}
                }
              ...
              >
                <div
                  className={`w-12 h-12 rounded-2xl flex items-center justify-center ${item.dark ? "bg-white/10" : item.bg}`}
                >
                  <item.icon
                    className={`w-6 h-6 ${item.dark ? "text-white" : item.iconColor}`}
                  />
                </div>
                <div className="flex-1">
                  <h4
                    className={`font-black text-base mb-1 ${item.dark ? "text-white" : "text-slate-900"}`}
                  >
                    {item.title}
                  </h4>
                  <p
                    className={`text-xs font-medium leading-relaxed ${item.dark ? "text-slate-400" : "text-slate-500"}`}
                  >
                    {item.desc}
                  </p>
                </div>
                {item.locked ? (
                  <span
                    className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-2 ${item.dark ? "text-slate-600" : "text-slate-300"}`}
                  >
      ...
      <div className="grid lg:grid-cols-3 gap-8 md:gap-10">
        ...
        >
          {hasStarted ? (
            <>
              ...
              <div
                className={`flex items-center justify-between mb-8 relative z-10 ${dir === "rtl" ? "flex-row-reverse" : ""}`}
              >
                <div>
                  <h2
                    className={`text-2xl font-black text-slate-900 flex items-center gap-3 ${dir === "rtl" ? "flex-row-reverse" : ""}`}
                  >
              ...
              <div
                className={`flex-1 ${isDiagnosisComplete ? "bg-linear-to-br from-blue-50 to-white border-blue-100" : "bg-linear-to-br from-slate-50 to-white border-slate-100"} rounded-3xl p-6 md:p-8 flex flex-col items-stretch gap-6 border relative ${dir === "rtl" ? "text-right" : "text-left"}`}
              >
                <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
                  <div className="flex-1 space-y-3 w-full">
                    <div className="flex flex-wrap items-center gap-3">
                      <span
                        className={`inline-block px-4 py-1.5 ${isDiagnosisComplete ? "bg-blue-600 text-white" : "bg-slate-200 text-slate-700"} text-[10px] font-black rounded-full uppercase tracking-widest shadow-sm`}
                      >
                        {isDiagnosisComplete
                          ? t.dashboard.diagnosisStatus.completed
                          : t.dashboard.diagnosisStatus.inProgress}
                      </span>
                      {isDiagnosisComplete && (
                        <span className="flex items-center gap-1.5 px-4 py-1.5 bg-emerald-100 text-emerald-700 text-[10px] font-black rounded-full uppercase tracking-widest">
                          ...
                          {t.dashboard.diagnosisStatus.successfullyCompleted}
                        </span>
                      )}
                    </div>
                    <h3 className="text-2xl font-black text-slate-900 leading-tight">
                      {isDiagnosisComplete
                        ? t.dashboard.diagnosisStatus.complete
                        : t.dashboard.diagnosisStatus.pending}
                    </h3>
                ...
                <div className="mt-4 pt-6 border-t border-slate-100 space-y-4">
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div
                      className={`p-4 bg-white border border-slate-100 rounded-2xl shadow-sm hover:shadow-md transition-all flex flex-col gap-2 ${dir === "rtl" ? "text-right" : "text-left"}`}
                    >
                      <div
                      ...
                      >
                        ...
                        <span className="ms-auto text-[8px] font-black px-2 py-0.5 bg-blue-50 text-blue-600 rounded-full uppercase tracking-tighter">
                          Auto-Activated
                        </span>
                      </div>
                    ...
                    <div
                      className={`p-4 bg-slate-900 border border-slate-800 rounded-2xl shadow-xl flex flex-col gap-2 ${dir === "rtl" ? "text-right" : "text-left"}`}
                    >
                      <div
                        className={`flex items-center gap-2 ${dir === "rtl" ? "flex-row-reverse" : ""}`}
                      >
                        ...
                        <span className="ms-auto text-[8px] font-black px-2 py-0.5 bg-amber-500 text-white rounded-full uppercase tracking-tighter">
                          Expert Scheduled
                        </span>
                      </div>
                      <p className="text-[10px] text-slate-400 font-bold leading-relaxed">
                        {
                          t.dashboard.diagnosisStatus.moduleInfo
                            .humanDescription
                        }
                      </p>
                    </div>
                ...
                {(userRole === "Trial User" || userRole === "Free Tier") &&
                  isDiagnosisComplete && (
                        ...
                        <div>
                          ...
                          <div className="flex gap-4">
                            <div className="flex items-center gap-1.5 text-[10px] font-bold">
                              ...
                              {t.dashboard.upsell.item1}
                            </div>
                            <div className="flex items-center gap-1.5 text-[10px] font-bold">
                              <CheckCircle2 className="w-3 h-3 text-blue-300" />{" "}
                              {t.dashboard.upsell.item2}
                            </div>
                          </div>
                        </div>
                        <Link
                          href="/subscription"
                          className="shrink-0 px-4 py-2 bg-white text-indigo-700 rounded-xl text-[9px] font-black uppercase tracking-wider hover:bg-slate-50 transition-colors text-center"
                        >
                          Upgrade to Pro
                        </Link>
                      </div>
                ...
                <div className="mt-8 grid sm:grid-cols-2 gap-6">
                  {cvAnalysis.strengths && cvAnalysis.strengths.length > 0 && (
                    <div className="bg-emerald-50/50 rounded-3xl p-6 border border-emerald-100/50">
                      <p
                        className={`text-xs font-black text-emerald-600 uppercase tracking-widest mb-4 flex items-center gap-2 ${dir === "rtl" ? "flex-row-reverse" : ""}`}
                      >
                        <CheckCircle2 className="w-4 h-4" />
                        {t.dashboard.sections.strengths}
                      </p>
                      <ul
                        className={`space-y-2.5 ${dir === "rtl" ? "text-right" : ""}`}
                      >
                        {cvAnalysis.strengths.slice(0, 3).map((s, i) => (
                          <li
                            key={i}
                            className="text-sm text-emerald-900 font-bold flex items-start gap-3"
                          >
                  ...
                  {cvAnalysis.skills?.gaps &&
                    cvAnalysis.skills.gaps.length > 0 && (
                      <div className="bg-amber-50/50 rounded-3xl p-6 border border-amber-100/50">
                        <p
                          className={`text-xs font-black text-amber-600 uppercase tracking-widest mb-4 flex items-center gap-2 ${dir === "rtl" ? "flex-row-reverse" : ""}`}
                        >
                        ...
                        >
                          {cvAnalysis.skills.gaps.slice(0, 3).map((g, i) => (
                            <li
                              key={i}
                              className="text-sm text-amber-900 font-bold flex items-start gap-3"
                            >
                  ...
                  {interviewEval?.executiveSummary && (
                    <div className="sm:col-span-2 bg-indigo-50/50 rounded-3xl p-6 border border-indigo-100/50">
                      <p
                        className={`text-xs font-black text-indigo-600 uppercase tracking-widest mb-3 flex items-center gap-2 ${dir === "rtl" ? "flex-row-reverse" : ""}`}
                      >
                  ...
                  {/* Immediate Actions */}
                  {cvAnalysis.immediateActions &&
                    cvAnalysis.immediateActions.length > 0 && (
                      <div className="sm:col-span-2 bg-slate-50/50 rounded-3xl p-6 border border-slate-100/50">
                        <p
                          className={`text-xs font-black text-slate-500 uppercase tracking-widest mb-4 flex items-center gap-2 ${dir === "rtl" ? "flex-row-reverse" : ""}`}
                        >
                        ...
                        <div className="flex flex-wrap gap-3">
                          {cvAnalysis.immediateActions
                            ...
                            .map((action, i) => (
                              <span
                                key={i}
                                className="text-xs px-4 py-2 bg-white border border-slate-200 rounded-xl text-slate-700 font-bold shadow-sm"
                              >
                                {action}
                              </span>
                            ))}
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center space-y-8">
              ...
              <Link
                href="/assessment/cv-upload"
                className="px-10 py-5 bg-blue-600 text-white rounded-2xl font-black shadow-2xl shadow-blue-600/30 hover:bg-blue-700 hover:scale-105 transition-all text-sm uppercase tracking-widest flex items-center gap-3"
              >
        ...
        >
          ...
          <div className="relative space-y-8">
            {/* Timeline line */}
            <div
              className={`absolute top-0 bottom-0 w-px bg-slate-800 ${dir === "rtl" ? "right-6" : "left-6"} top-6 bottom-6`}
            />
            ...
            {steps.map((step, idx) => (
              <div key={idx} className="relative group">
                ...
                >
                  {/* Stage Indicator */}
                  <div
                    className={`relative z-10 w-12 h-12 shrink-0 rounded-2xl flex items-center justify-center border-2 transition-all duration-500 
                                        ${
                  ...
                  <div className="flex-1 space-y-1">
                    ...
                    >
                      <h3
                        className={`font-black text-base transition-colors duration-300 ${step.status === "locked" ? "text-slate-600" : "text-white group-hover:text-blue-400"}`}
                      >
                        {step.title}
                      </h3>
                      {step.status === "in-progress" && (
                    ...
                    <p
                      className={`text-xs leading-relaxed font-medium transition-colors ${step.status === "locked" ? "text-slate-700" : "text-slate-400"}`}
                    >
                      {step.description}
                    </p>
                    ...
                    {step.status !== "locked" && (
                      <div className="overflow-hidden pt-2 space-y-3">
                        <div className="p-3 bg-white/5 rounded-xl border border-white/5 space-y-2 group-hover:border-white/10 transition-colors">
                          <div
                            className={`flex items-start gap-2 ${dir === "rtl" ? "flex-row-reverse text-right" : ""}`}
                          >
                            <Sparkles
                              size={12}
      ...
      {liveSessions.length > 0 && (
        ...
        >
          ...
          <div
            className={`relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8 ${dir === "rtl" ? "flex-row-reverse" : ""}`}
          >
            <div className="space-y-4">
              ...
              <h2
                className={`text-2xl md:text-3xl font-black text-white ${dir === "rtl" ? "text-right" : ""}`}
              >
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mt-10 relative z-10">
            {liveSessions.map((session, idx) => (
              <div
                key={idx}
                className={`bg-white/5 backdrop-blur-md border border-white/10 p-6 rounded-3xl hover:bg-white/10 transition-all group ${dir === "rtl" ? "text-right" : ""}`}
              >
                <div
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-slate-400 text-xs">
                      {t.dashboard.liveSessions.expert}: {session.expertName}
                    </p>
                  </div>
                </div>
                <div className="space-y-3 mb-8">
                  <div
                    className={`flex items-center gap-3 text-slate-300 text-sm ${dir === "rtl" ? "flex-row-reverse" : ""}`}
                  >
