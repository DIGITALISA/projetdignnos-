...
export default function RecruitmentManagement() {
    ...
    const [expertInterviews, setExpertInterviews] = useState<ExpertInterview[]>([]);
    const [isLoadingInterviews, setIsLoadingInterviews] = useState(true);
    const [selectedInterview, setSelectedInterview] = useState<ExpertInterview | null>(null);
    ...
    const fetchExpertInterviews = async () => {
        setIsLoadingInterviews(true);
        try {
            const res = await fetch("/api/expert-interview");
            const data = await res.json();
            if (data.success) {
                setExpertInterviews(data.data);
            }
        } catch (error) {
            console.error("Error fetching expert interviews:", error);
        } finally {
    ...
    const handleDeleteInterview = async (id: string) => {
        ...
        try {
            const res = await fetch(`/api/expert-interview?id=${id}`, { method: "DELETE" });
            if (res.ok) {
                setExpertInterviews(prev => prev.filter(i => i._id !== id));
                if (selectedInterview?._id === id) setSelectedInterview(null);
            }
    ...
    const updateInterviewStatus = async (id: string, newStatus: string) => {
        try {
            ...
            if (res.ok) {
                setExpertInterviews(prev => prev.map(i => i._id === id ? { ...i, status: newStatus } : i));
                if (selectedInterview?._id === id) setSelectedInterview({ ...selectedInterview, status: newStatus });
            }
        } catch (error) { console.error(error); }
    };
    ...
    const filteredInterviews = expertInterviews.filter(i => {
        const matchesSearch = `${i.firstName} ${i.lastName}`.toLowerCase().includes(searchTerm.toLowerCase()) || 
        ...
        <div className="space-y-16 p-4 md:p-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="flex items-center gap-3">
                    <button onClick={() => { fetchApplications(); fetchExpertInterviews(); }} className="p-3 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors">
                        <Loader2 size={20} className={cn((isLoading || isLoadingInterviews) && "animate-spin")} />
                    </button>
                </div>
            </div>
                ...
                <div className="flex flex-col lg:flex-row items-center gap-4">
                    ...
                    <div className="flex items-center gap-3 w-full lg:w-auto overflow-x-auto pb-2 lg:pb-0">
                        <select 
                            ...
                            className="px-4 py-3 bg-slate-50 border-none rounded-2xl text-[10px] font-bold uppercase tracking-widest outline-none appearance-none cursor-pointer"
                        >
            ...
            <section className="space-y-6">
                <div className="flex items-center gap-4 border-b border-slate-100 pb-4">
                    ...
                    <h2 className="text-xl font-bold text-slate-900 uppercase tracking-tighter">Applications Hub (CV & Video)</h2>
                </div>
                {/* Grid Apps */}
            ...
            <section className="space-y-6">
                <div className="flex items-center gap-4 border-b border-slate-100 pb-4">
                    ...
                    <h2 className="text-xl font-bold text-slate-900 uppercase tracking-tighter">Strategic Expert Interviews (16 Qs)</h2>
                </div>
                {/* Grid Interviews */}
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    {/* List */}
                    <div className="lg:col-span-12 xl:col-span-8 space-y-4">
                        {isLoadingInterviews ? (
                            <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-slate-100">
                                ...
                                <p className="text-slate-500 font-bold uppercase tracking-widest text-xs">Loading Expert Interviews...</p>
                            </div>
                        ) : filteredInterviews.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-slate-100">
                                <Clock size={40} className="text-slate-200 mb-4" />
                                <p className="text-slate-500 font-bold uppercase tracking-widest text-xs">No interviews matching criteria.</p>
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {filteredInterviews.map((interview) => (
                                    ...
                                    >
                                        <div className="flex items-start justify-between gap-4 relative z-10">
                                            <div className="flex items-center gap-4">
                                                <div className="w-14 h-14 rounded-2xl bg-indigo-50 text-indigo-600 flex items-center justify-center font-bold text-xl overflow-hidden">
                                                    {interview.firstName.charAt(0)}
                                                </div>
                                                <div>
                                                    <h3 className="font-bold text-slate-900 group-hover:text-indigo-600 transition-colors uppercase tracking-tight">{interview.firstName} {interview.lastName}</h3>
                                                    <p className="text-xs font-black text-slate-400 uppercase tracking-widest mt-1">Expert Interview</p>
                                                </div>
                                            </div>
                                            <div className={cn(
                                                "px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-tighter border",
                                        ...
                                        <div className="mt-6 flex flex-wrap gap-2">
                                            <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-50 text-slate-500 rounded-xl text-[10px] font-bold">
                                                <Mail size={12} />
                                                {interview.email.substring(0, 20)}...
                                            </div>
                                            <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-50 text-slate-500 rounded-xl text-[10px] font-bold">
                                                <Clock size={12} />
                    ...
                    <AnimatePresence>
                        {selectedInterview && (
                            ...
                            >
                                <div className="bg-white rounded-4xl border border-slate-100 shadow-2xl p-8 sticky top-8 space-y-8 overflow-hidden">
                                    <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-600/5 rounded-bl-full -mr-8 -mt-8" />
                                    
                                    <div className="flex items-start justify-between relative z-10">
                                        <div className="space-y-1">
                                            <h2 className="text-2xl font-bold text-slate-900">{selectedInterview.firstName} {selectedInterview.lastName}</h2>
                                            <p className="text-sm text-indigo-600 font-black uppercase tracking-[0.2em]">Expert Candidate</p>
                                        </div>
                                        <button onClick={() => setSelectedInterview(null)} className="p-2 hover:bg-slate-50 rounded-xl transition-colors">
                                            <X size={20} className="text-slate-400" />
                                        </button>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100">
                                            ...
                                            <select 
                                                ...
                                                onChange={(e) => updateInterviewStatus(selectedInterview._id, e.target.value)}
                                                className="w-full bg-transparent border-none p-0 text-sm font-bold text-slate-900 focus:ring-0 cursor-pointer"
                                            >
                                                <option value="Pending">Pending</option>
                                                <option value="Reviewed">Reviewed</option>
                                        ...
                                        <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <Phone size={14} className="text-emerald-500" />
                                                <span className="text-sm font-bold">{selectedInterview.phone}</span>
                                            </div>
                                        </div>
                                    </div>

                                    <div className="space-y-6">
                                        <p className="text-xs font-black text-slate-400 uppercase tracking-widest pb-2 border-b border-slate-100">Payment Preference</p>
                                        <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                                            <div className="flex items-center justify-between">
                                                <div className="space-y-1">
                                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Mode</p>
                                                    <p className="text-lg font-black text-slate-900">{selectedInterview.paymentMode}</p>
                                                </div>
                                                {selectedInterview.amount && (
                                                    <div className="text-right">
                                                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Amount</p>
                                                        <p className="text-xl font-black text-indigo-600">{selectedInterview.amount} <span className="text-sm">DZD</span></p>
                                                    </div>
                                                )}
                                            </div>
                                            {selectedInterview.paymentOther && (
                                                <div className="mt-4 pt-4 border-t border-slate-200">
                                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Details</p>
                                                    <p className="text-sm font-bold text-slate-700">{selectedInterview.paymentOther}</p>
                                                </div>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                        <p className="text-xs font-black text-slate-400 uppercase tracking-widest pb-2 border-b border-slate-100">Interview Answers (16 Qs)</p>
                                        <div className="grid grid-cols-1 gap-4 overflow-y-auto max-h-[400px] pr-2 custom-scrollbar">
                                            {[
                                                "Expertise principal",
                                                "Années d'expérience",
                                                ...
                                                const answer = selectedInterview.answers[idx.toString()];
                                                if (!answer) return null;
                                                return (
                                                    <div key={idx} className="space-y-1">
                                                        <div className="flex items-center gap-2">
                                                            <span className="text-[10px] font-black text-indigo-600">Q{idx + 1}</span>
                                                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-tighter">{q}</p>
                                                        </div>
                                                        <p className="text-sm font-medium text-slate-700 leading-relaxed bg-slate-50/80 p-3 rounded-xl border border-slate-100/50">
                                                            {answer}
                                                        </p>
