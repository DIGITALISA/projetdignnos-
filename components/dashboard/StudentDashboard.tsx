...
export function StudentDashboard({ 
    ...
    const localT = {
        ar: {
            ...
            nextStepsTitle: "خطواتك القادمة لمحاكاة الحقيقة",
            nextStepsDesc: "الآن وبعد إتمام الديغنوستيك، اختر أحد المسارات المتقدمة لبناء مهارات عملية وحقيقية.",
            workshopCard: "ورشات عمل تخصصية",
            workshopBadge: "احتراف المهارة",
            simCard: "محاكاة الدور الوظيفي",
            simBadge: "تجربة القيادة",
            getStarted: "ابدأ الآن",
            roadmap: [
                { title: "الحالم", sub: "إعداد الملف" },
                { title: "المستكشف", sub: "الديغنوستيك" },
                { title: "البنّاء", sub: "ورشات العمل" },
                { title: "المحارب", sub: "المحاكاة" },
                { title: "المحترف", sub: "عقود العمل" },
            ]
        },
        en: {
             // ... keeping en translations
            ...
            nextStepsTitle: "Professional Next Steps",
            nextStepsDesc: "Diagnosis complete! Now choose a specialized path to build real-world mastery.",
            workshopCard: "Expert Workshops",
            workshopBadge: "Skill Mastery",
            simCard: "Role Simulation",
            simBadge: "Real Experience",
            getStarted: "Get Started",
            roadmap: [
                { title: "Dreamer", sub: "Setup" },
                { title: "Scout", sub: "Diagnostic" },
        ...
        fr: {
            ...
            nextStepsDesc: "Diagnostic terminé ! Choisissez maintenant un parcours spécialisé pour maîtriser le terrain.",
            workshopCard: "Ateliers d'Experts",
            workshopBadge: "Maîtrise Skill",
            simCard: "Simulation de Rôle",
            simBadge: "Expérience Réelle",
            getStarted: "Commencer",
            roadmap: [
                { title: "Rêveur", sub: "Profil" },
                { title: "Éclaireur", sub: "Diagnostic" },
    ...
    const local = localT[currentLang as keyof typeof localT] || localT.en;
    ...
    return (
        <div className={cn("space-y-12 pb-24", isRtl ? "text-right" : "text-left")}>
            ...
            {/* --- 2. Post-Diagnostic: Next Steps (Paid Opportunities) --- */}
            {isDiagnosisComplete && (
                <motion.section 
                ...
                >
                    <div className={cn("flex flex-col md:flex-row md:items-end justify-between gap-4", isRtl ? "md:flex-row-reverse" : "")}>
                        <div className="space-y-2">
                             <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-indigo-50 text-indigo-700 rounded-full text-[10px] font-black uppercase tracking-widest border border-indigo-100 mb-2">
                                <Rocket className="w-3.5 h-3.5" />
                                {currentLang === 'ar' ? 'أنت جاهز للمرحلة التالية' : currentLang === 'fr' ? 'Prêt pour la suite' : 'Ready for the next stage'}
                            </div>
                            <h2 className="text-4xl font-black text-slate-900 tracking-tight">{local.nextStepsTitle}</h2>
                            <p className="text-slate-500 font-bold text-sm max-w-2xl">{local.nextStepsDesc}</p>
                        </div>
                    </div>

                    <div className="grid md:grid-cols-2 gap-8">
                        {/* Workshops Card */}
                        <div className="group relative bg-white rounded-[2.5rem] p-10 border border-slate-200 shadow-xl hover:shadow-2xl hover:border-indigo-200 transition-all overflow-hidden">
                            <div className="absolute top-0 right-0 p-10 opacity-5 group-hover:opacity-10 transition-opacity">
                                <PlayCircle className="w-40 h-40" />
                            </div>
                            <div className="relative z-10 space-y-8">
                                <div className="flex justify-between items-start">
                                    <div className="w-16 h-16 bg-indigo-600 text-white rounded-3xl flex items-center justify-center shadow-lg shadow-indigo-200 group-hover:scale-110 transition-transform">
                                        <PlayCircle className="w-8 h-8" />
                                    </div>
                                    <span className="bg-indigo-50 text-indigo-600 text-[10px] font-black px-3 py-1 rounded-full uppercase border border-indigo-100">
                                        {local.workshopBadge}
                                    </span>
                                </div>
                                <div>
                                    <h3 className="text-3xl font-black text-slate-900 mb-2">{local.workshopCard}</h3>
                                    <p className="text-slate-500 font-medium text-sm leading-relaxed">
                                        {currentLang === 'ar' ? 'تعلم الممارسات المهنية الحقيقية من الخبراء واختصر سنوات من الخطأ والصواب.' : 
                                         currentLang === 'fr' ? 'Apprenez les pratiques réelles auprès d\'experts et évitez des années de tâtonnements.' : 
                                         'Learn real-world professional practices from experts and skip years of trial and error.'}
                                    </p>
                                </div>
                                <Link 
                                    href="/training"
                                    className="flex items-center justify-between w-full p-4 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] group-hover:bg-indigo-600 transition-colors"
                                >
                                    {local.getStarted}
                        ...
                        {/* Simulation Card */}
                        <div className="group relative bg-slate-900 rounded-[2.5rem] p-10 text-white shadow-2xl hover:shadow-[0_20px_60px_rgba(37,99,235,0.3)] transition-all overflow-hidden">
                            <div className="absolute bottom-0 right-0 p-10 opacity-10 group-hover:opacity-20 transition-opacity">
                                <Zap className="w-40 h-40" />
                            </div>
                            <div className="relative z-10 space-y-8">
                                <div className="flex justify-between items-start">
                                    <div className="w-16 h-16 bg-blue-600 text-white rounded-3xl flex items-center justify-center shadow-lg shadow-blue-500/20 group-hover:scale-110 transition-transform">
                                        <Zap className="w-8 h-8" />
                                    </div>
                                    <span className="bg-white/10 text-blue-300 text-[10px] font-black px-3 py-1 rounded-full uppercase border border-white/10">
                                        {local.simBadge}
                                    </span>
                                </div>
                                <div>
                                    <h3 className="text-3xl font-black text-white mb-2">{local.simCard}</h3>
                                    <p className="text-slate-400 font-medium text-sm leading-relaxed">
                                        {currentLang === 'ar' ? 'عش تجربة يوم عمل حقيقي في كبرى الشركات واختبر قدراتك القيادية والعملية.' : 
                                         currentLang === 'fr' ? 'Vivez une véritable journée de travail au sein de grandes entreprises et testez vos capacités.' : 
                                         'Experience a real work day in top companies and test your leadership and operational skills.'}
                                    </p>
                                </div>
                                <Link 
                                    href="/simulation"
                                    className="flex items-center justify-between w-full p-4 bg-white text-slate-900 rounded-2xl font-black text-xs uppercase tracking-[0.2em] group-hover:bg-blue-600 group-hover:text-white transition-all"
                                >
                                    {local.getStarted}
                                    <ArrowRight className={cn("w-5 h-5", isRtl && "rotate-180")} />
                                </Link>
