...
path = r'c:\Users\ahmed\Desktop\e-projet\lib\deepseek.ts'
with open(path, 'r', encoding='utf-8') as f:
    content = f.read()

new_system_prompt = """You are a legendary Executive HR Auditor with 50 years of experience assessing high-potential leaders. 
...
# Regex to find the content of the system prompt in generateProfessionalInterviewQuestion
# It matches the message array within that specific function
pattern = re.compile(r'(export async function generateProfessionalInterviewQuestion.*?\n\s+messages: \[\s+\{\s+role: \'system\',\s+content: `).*?(`\s+\},)', re.DOTALL)

def replacement(match):
    return match.group(1) + new_system_prompt + match.group(2)
